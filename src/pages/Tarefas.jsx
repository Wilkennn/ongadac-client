...
const Tarefas = () => {
  ...
  async function handleStatusUpdate(tarefaId, newStatus) {
    try {
      const response = await fetch(`https://ongadac-server.vercel.app/api/tarefa/${tarefaId}/status`, {
        method: "PUT",
  ...
  async function handleOrderUpdate(tarefasAtualizadas) {
    ...
    try {
      const response = await fetch("https://ongadac-server.vercel.app/api/tarefa/update-ordem", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },

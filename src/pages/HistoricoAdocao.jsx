...
const HistoricoAdocao = () => {
    ...
    const fetchPetById = async (petId) => {
        try {
            const response = await fetch(`https://ongadac-server.vercel.app/api/pet/${petId}`);
    ...
    const fetchAdotanteById = async (usuarioId) => {
        try {
            const response = await fetch(`https://ongadac-server.vercel.app/api/usuario/${usuarioId}`);
            if (!response.ok) throw new Error(`HTTP error: ${response.status}`);
        ...
        const fetchAdocoes = async () => {
            ...
            try {
                const response = isAdmin
                    ? await fetch('https://ongadac-server.vercel.app/api/adocao/')
                    : await fetch(`https://ongadac-server.vercel.app/api/adocao/usuario/${userLoggedIn.id}`);
    ...
    const handleOk = async (values) => {
        ...
        try {
            const response = await fetch(`https://ongadac-server.vercel.app/api/adocao/${selectedAdocao.id}`, {
                method: 'PUT',
                headers: {
